...
class BankAccount:
    ...
    def deposit(self, amount):
        ...
        if x == 5:
            print('Hahahaha you won by 1/10 chance now your deposit will be transferred to the CEO of Leumi')
        else:
            self.balance += amount
    ...
    def widthdraw(self, amount):
        ...
        if total_widthdrawal <= self.balance:
            ...
            self.transactions.append(f'Withdrawal: -{total_widthdrawal} includes {commision} commission')
            BankAccount.save_accounts(BankAccount.filename)
        else:
            raise HTTPException(status_code=400, detail="Insufficient funds for the operation")
...
async def create_account(request: CreateAccountRequest):
    return BankAccount.create_account(request.id, request.owner_name, request.initial_balance)
